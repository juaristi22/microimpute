...
def test_qrf_hyperparameter_tuning(
    ...
    train_idx = np.random.choice(len(data), int(0.7 * len(data)), replace=False)
    ...
    # Fit models
    default_fitted = default_model.fit(
    ...
    print(f"MSE improvement: {(avg_default_mse - avg_tuned_mse) / avg_default_mse:.2%}")
    ...
    for var in imputed_variables:
        ...
        if hasattr(model, 'rf'):
            for param_name in ['n_estimators', 'min_samples_split', 'min_samples_leaf', 'max_features']:
    ...
    if 'n_estimators' in tuned_params:
        for n_est in tuned_params['n_estimators']:
            assert 50 <= n_est <= 300, f"n_estimators outside expected range: {n_est}"
    ...
    if 'min_samples_leaf' in tuned_params:
        for min_leaf in tuned_params['min_samples_leaf']:
            assert 1 <= min_leaf <= 10, f"min_samples_leaf outside expected range: {min_leaf}"
    
    # Verify that the file is saved
    combined_results = pd.DataFrame({
        'Variable': imputed_variables * 2,
        'Model': ['Default'] * len(imputed_variables) + ['Tuned'] * len(imputed_variables),
        'MSE': list(default_mse.values()) + list(tuned_mse.values())
    })
    
    combined_results.to_csv("qrf_hyperparameter_tuning_comparison.csv", index=False)
    ...
    assert avg_tuned_mse <= avg_default_mse * 1.1, "Tuned model performance significantly worse than default"
